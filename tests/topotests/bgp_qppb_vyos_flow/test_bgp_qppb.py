...
def setup_test_hosts(tgen, router):
    ...
    """
    h1 = tgen.add_host("h1", "20.0.0.1", "dev h1-eth0", privateDirs="")
    h2 = tgen.add_host("h2", "20.0.1.1", "dev h2-eth0", privateDirs="")
    router.add_link(h1)
    router.add_link(h2)
...
def test_xdp_lpm(tgen):
    ...
    def _check(sender, receiver, dst, cap_iface, tos, src=None, ping_tos=None):
        ...
        p2 = check_ping4(sender, dst, src=src, count=PINGS, timeout=PINGS, tos=ping_tos)
        time.sleep(1.5)
...
def test_xdp_network_overlap(tgen):
    """
    The feature configuration involves quite a bit of steps making it quite
    easy to messup and accidentally leak traffic, apply wrong preference, etc ..
...
# def test_dscp_to_vrf (TBD: next in the priority)
# def test_qos_over_l3_dev (vrf/bridge/lag/..., check what cumulus/melanox for examples)
# def test_with_fragmentation
# def test_with_tunnels
# def test_defaulte_route_mark  (does this make sense?)
# def test_loader (assuming there will some kind of loader choosen)
#
# def test_scalability (TBD)
# def test_limits | what table sizes should be? lpm/qppb_mode
#                 | what if we run out of prefixes in lpm (abort mechanism?)
#
#
# @pytest.skipIf( '-quick' in sys.argv, 'long test' )
# def test_stability()
#     r1[iperf]  (5 min) -> r3
#     assert all_packets_marked
#            ......
